...
for (t1=-1;t1<=1;t1++) {
  lbp=max(0,t1);
  ubp=floord(t1+3125,2);
#pragma omp parallel for private(lbv,ubv,t3,t4)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=max(max(0,512*t1),1024*t1-1024*t2+1);t3<=min(999,512*t1+1023);t3++) {
      lbv=max(max(1024*t2,t3+1),-1024*t1+1024*t2+2*t3-1023);
      ubv=min(min(1024*t2+1023,t3+1600000),-1024*t1+1024*t2+2*t3);
#pragma ivdep
