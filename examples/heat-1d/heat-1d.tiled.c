...
for (t1=-1563;t1<=0;t1++) {
  for (t2=max(t1,-t1-1);t2<=min(-t1+1,t1+3125);t2++) {
    for (t3=max(max(0,512*t1+512*t2),1024*t1+1);t3<=min(999,512*t1+512*t2+1023);t3++) {
      lbv=max(max(1024*t2,t3+1),-1024*t1+2*t3-1023);
      ubv=min(min(-1024*t1+2*t3,1024*t2+1023),t3+1600000);
#pragma ivdep
