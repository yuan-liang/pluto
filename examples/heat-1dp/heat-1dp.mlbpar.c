...
if (_T >= 1) {
  for (t1=-1;t1<=floord(_T-1,512);t1++) {
    lbp=ceild(t1,2);
    ubp=min(floord(t1+1563,2),floord(_T+799998,1024));
#pragma omp parallel for private(lbv,ubv,t3,t4)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(0,512*t1),1024*t2-799999);t3<=min(min(_T-1,512*t1+1023),1024*t1-1024*t2+801022);t3++) {
        lbv=max(max(1024*t2,t3),-1024*t1+1024*t2+2*t3-1023);
        ubv=min(min(1024*t2+1023,t3+799999),-1024*t1+1024*t2+2*t3);
#pragma ivdep
