...
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
  for (t2=lbp;t2<=ubp;t2++) {
    for (t3=max(0,ceild(t1-1,2));t3<=floord(t1+126,2);t3++) {
      for (t4=max(max(max(0,32*t1),64*t3-4000),64*t1-64*t2+1);t4<=min(min(min(999,32*t1+63),64*t2+62),64*t3+62);t4++) {
        for (t5=max(max(64*t2,t4+1),-64*t1+64*t2+2*t4-63);t5<=min(min(64*t2+63,t4+4000),-64*t1+64*t2+2*t4);t5++) {
          lbv=max(64*t3,t4+1);
          ubv=min(64*t3+63,t4+4000);
#pragma ivdep
