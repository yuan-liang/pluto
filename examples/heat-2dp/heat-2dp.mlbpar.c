...
if (_T >= 1) {
  for (t1=-14;t1<=floord(_T-1,32);t1++) {
    lbp=max(0,ceild(t1,2));
    ubp=min(min(floord(t1+38,2),floord(_T+798,64)),t1+25);
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(max(max(0,ceild(t1-1,2)),-t1-2),-t1+3*t2-39);t3<=min(min(min(min(floord(t1+39,2),floord(-32*t1+32*t2+_T-1,32)),floord(_T+798,64)),floord(-64*t1+3*_T-3,64)),-t1+3*t2+1);t3++) {
        for (t4=max(max(max(max(max(0,ceild(64*t1+64*t3,3)),32*t1),64*t2-799),64*t3-799),32*t1-32*t2+32*t3);t4<=min(min(min(min(min(min(floord(64*t1+64*t3+189,3),_T-1),32*t1+462),64*t2+63),64*t3+63),64*t1-64*t2+1661),32*t1-32*t2+32*t3+462);t4++) {
          for (t5=max(max(max(64*t2,t4),-64*t1+64*t2+2*t4-862),-64*t1+64*t2-64*t3+3*t4-126);t5<=min(min(min(64*t2+63,t4+799),-64*t1+64*t2+2*t4),-64*t1+64*t2-64*t3+3*t4);t5++) {
            lbv=max(max(64*t3,t4),-64*t1+64*t2+3*t4-t5-63);
            ubv=min(min(64*t3+63,t4+799),-64*t1+64*t2+3*t4-t5);
#pragma ivdep
