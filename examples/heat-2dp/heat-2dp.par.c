...
if (_T >= 1) {
  for (t1=-1;t1<=floord(_T-1,32);t1++) {
    lbp=ceild(t1,2);
    ubp=min(floord(t1+25,2),floord(_T+798,64));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(0,ceild(t1-1,2));t3<=min(floord(t1+26,2),floord(_T+798,64));t3++) {
        for (t4=max(max(max(0,32*t1),64*t2-799),64*t3-799);t4<=min(min(min(_T-1,32*t1+63),64*t3+63),64*t1-64*t2+862);t4++) {
          for (t5=max(max(64*t2,t4),-64*t1+64*t2+2*t4-63);t5<=min(min(64*t2+63,t4+799),-64*t1+64*t2+2*t4);t5++) {
            lbv=max(64*t3,t4);
