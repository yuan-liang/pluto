...
if (_T >= 1) {
  for (t1=-1;t1<=floord(_T-1,8);t1++) {
    lbp=ceild(t1,2);
    ubp=min(floord(t1+19,2),floord(_T+148,16));
#pragma omp parallel for private(lbv,ubv,t3,t4,t5,t6,t7)
    for (t2=lbp;t2<=ubp;t2++) {
      for (t3=max(0,ceild(t1-1,2));t3<=min(floord(t1+20,2),floord(_T+148,16));t3++) {
        for (t4=max(max(max(0,8*t1),16*t2-149),16*t3-149);t4<=min(min(min(_T-1,8*t1+15),16*t3+15),16*t1-16*t2+164);t4++) {
          for (t5=max(max(16*t2,t4),-16*t1+16*t2+2*t4-15);t5<=min(min(16*t2+15,t4+149),-16*t1+16*t2+2*t4);t5++) {
