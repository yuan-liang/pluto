...
if (_T >= 1) {
  for (t1=-10;t1<=floord(_T-1,16);t1++) {
    for (t2=max(t1,-t1-1);t2<=min(min(floord(-8*t1+_T-1,8),floord(_T+148,16)),t1+19);t2++) {
      for (t3=max(0,ceild(t1+t2-1,2));t3<=min(floord(t1+t2+20,2),floord(_T+148,16));t3++) {
        for (t4=max(max(max(0,8*t1+8*t2),16*t2-149),16*t3-149);t4<=min(min(min(_T-1,16*t1+164),16*t3+15),8*t1+8*t2+15);t4++) {
          for (t5=max(max(16*t2,t4),-16*t1+2*t4-15);t5<=min(min(-16*t1+2*t4,16*t2+15),t4+149);t5++) {
            for (t6=max(16*t3,t4);t6<=min(16*t3+15,t4+149);t6++) {
              lbv=0;
              ubv=299;
#pragma ivdep
